...
-- (Inserções de dados mantidas como estão)
...
SELECT name, population
FROM world
WHERE name IN ('Poland', 'Spain', 'Netherlands');

-- BETWEEN: intervalo de valores
SELECT name, area
FROM world
WHERE area BETWEEN 300000 AND 400000;
...
FROM world
WHERE population > (
    SELECT population FROM world WHERE name = 'Cuba'
);
...
WHERE continent = 'Europe'
  AND gdp / population > (
      SELECT gdp / population FROM world WHERE name = 'Germany'
);
...
SELECT name,
       CONCAT(ROUND(population / (
           SELECT population FROM world WHERE name = 'Germany') * 100), '%') AS perc_vs_germany
FROM world
WHERE continent = 'Europe';
